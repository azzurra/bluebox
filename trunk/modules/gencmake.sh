...
function library_add()
{
  ...
  echo "SET_TARGET_PROPERTIES($CURRNAME PROPERTIES PREFIX \"\" SUFFIX \${SHLIBEXT})"
  echo "INSTALL(TARGETS $CURRNAME DESTINATION modules${MODPATH2})"
  echo "IF(APPLE)"
  echo "  TARGET_LINK_LIBRARIES($CURRNAME ratboxcore ratbox)"
  echo "ENDIF(APPLE)"
  echo 
}
